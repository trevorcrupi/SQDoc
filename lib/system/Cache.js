...
class Cache extends FileWriter {
  ...
  **/
  cache(filePathArray) {
    ...
    if(!this.exists(path))
      return { created: false, err: 'File already exists!' };

    // Filename is the last member of the array
    const filepath = this.join([this.projectPath, filePathArray[filePathArray.length-1]+'.handlebars']);
    return this.create({
      path: filepath,
  ...
  **/
  retrieveFullCachePath(name) {
    return this.join([this.projectPath, name + '.handlebars']);
  }
}
