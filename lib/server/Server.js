...
/**
  ...
  @param globals: Bad way of saying all the data from the .lock file
**/

class Server {

  constructor(structure, globals) {
  ...
  register(build) {
    this.tree.forEach((route) => {
      ...
      const cachePath = this.reader.getCachePath(this.reader.join(this.globals.projectNameTrim, route.name));
