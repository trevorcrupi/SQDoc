...
class Router {
  ...
  registerDirectoryRoute(route, isCached) {
    ...
    console.log('[ROUTING] Route registered for %s', colors.green(uri));
    this.app.get(uri, (req, res) => {
      res.render('<h1>This is a directory.</h1>' + route.name);
    });
  }

  cache(route) {
    console.log('[CACHE] Found uncached markdown file %s, converting contents to HTML file %s...', colors.green(route.name), colors.green(route.name + '.html'));
    try {
