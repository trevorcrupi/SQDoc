...
test('remove correctly returns when given good file', () => {
  const fullPath = path.join(process.cwd(), 'lib', 'system', 'cache', 'Test', 'test2.md.html');
  expect(system.Cache('Test').remove('test2.md.html')).toEqual({ removed: true, path: fullPath });
});

test('remove correctly returns false when given bad file', () => {
  const fullPath = path.join(process.cwd(), 'lib', 'system', 'cache', 'Test', 'test3.md.html');
  expect(system.Cache('Test').remove('test3.md.html').removed).toBe(false);
});
